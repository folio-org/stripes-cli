...
Example:
```
stripescli app create "Hello World" --install
```
...
* `--push` Push the app's module descriptior to Okapi and enable it for the current tenant (as defined by `--tenant`).
* `--assign [username]` Assign the app's default permissions to a user.

Example:
```
stripescli app create "Hello World" --install --push --assign someone
```

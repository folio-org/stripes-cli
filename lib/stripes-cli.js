...
commander
  .command('test')
  .option('--run [script]', 'name of test to run')
  .option('--devTools', 'Show test execution in browser window with DevTools opened')
  ...
  .action((options) => {
    ...
    const stripesConfig = loadConfig(null, context);

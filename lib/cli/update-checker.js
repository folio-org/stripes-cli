...
module.exports = class UpdateChecker {
  ...
  refreshIfStale() {
    ...
    const registry = this.packageJson.publishConfig?.registry || 'https://registry.npmjs.org/';
    const trimmedRegistry = registry.endsWith('/') ? registry.slice(0, -1) : registry;
    const url = `${trimmedRegistry}/${encodeURIComponent(this.packageJson.name)}`;

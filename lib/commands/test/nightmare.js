  ...
  const invokeTests = () => {
    ...
    return nightmareService.runNightmareTests(argv)
      .then(() => {
        ...
        if(argv.coverage) {
          nightmareService.runCoverageReport();
          /*.then(() =>{
            console.error('coverage reporting succeeded');
            process.exit(0);

          }).catch((err) =>{
            console.error('coverage reporting failed');
            console.error(err);
            process.exit(1);
      ...
      .catch((err) => {
        ...
        console.error(err);
        if(argv.coverage) {
          nightmareService.runCoverageReport().then(() =>{
            console.log("ran coverage");
            process.exit(0);
          })
          .catch(() => {
            console.log("error in coverage");
            process.exit(1);
          });
        }
        process.exit(1);

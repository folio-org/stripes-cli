...
const { validateMirageScenario, enableMirage } = require('../mirage/mirage-setup');
...
function serveCommand(argv) {
  ...
  if (argv.mirage && context.type === 'app') {
...
module.exports = {
  ...
  builder: (yargs) => {
    yargs
      ...
      .option('mirage', {
        describe: 'Use the mirage server to simulate the backend',
        type: 'string',
        coerce: validateMirageScenario,
      })

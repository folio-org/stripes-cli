...
function cleanCommand(argv) {
  ...
  if (argv.removeLock) {
    console.log(`Removing ${moduleDirs.length} lock file(s) ...`);
  }
  ...
  moduleDirs.forEach((dir) => {
    ...
    if (argv.removeLock) {
      packageManager.lockfiles.forEach((lockfile) => {
        clean.push(removeFileOrFolder(dir, lockfile));
      });
    }
  });
...
module.exports = {
  ...
  builder: (yargs) => {
    ...
    yargs.option('removeLock', {
      describe: 'Remove lock file(s) after cleaning',
      type: 'boolean',
      default: false,
    });
    yargs
      .example('$0 platform clean --install', 'Clean and reinstall dependencies')
      .example('$0 platform clean --removeLock', 'Clean and remove lock file(s)')

...
// Command (match[1]) begins with "stripes", description (match[2]) is separated by two new-lines
...
// Item name (match[1]) begins after space, description/details (match[2]) is separated by two or more spaces
const defaultItemRegex = /^\s+(.*?)\s{2,}\b(.*)$/gm;
...
function parseOption(optionMatch) {
  ...
  if (optionMatch.length > 2) {
    ...
    const defaultMatch = optionMatch[2].match(/\[default: (\w*)]/);    // [default: value]

...
function cliResolve(context) {
  return (config) => {
    if (context.isGlobalYarn) {
      ...
      config.resolve.modules.push(path.resolve(import.meta.dirname, path.join('..', 'node_modules')));
      config.resolveLoader.modules.push(path.resolve(import.meta.dirname, path.join('..', 'node_modules')));
    }
...
function enableCoverage(config) {
  ...
  if (!config.module.rules[babelLoaderConfigIndex]?.oneOf?.[1].use[0].options?.plugins) {
    _.set(config.module.rules[babelLoaderConfigIndex], 'oneOf[1].use[0].options.plugins', []);
  }
  ...
  _.set(config.module, `rules[${babelLoaderConfigIndex}]`, babelConfigurationItem);

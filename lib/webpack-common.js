...
function cliResolve(isLocalCoreAvailable, stripesCoreAlias) {
  return (config) => {
    ...
    if (stripesCoreAlias) {
      ...
      const aliasedCoreModulePath = path.join(stripesCoreAlias, 'node_modules');
      config.resolve.modules.push(aliasedCoreModulePath);
      config.resolveLoader.modules.push(aliasedCoreModulePath);

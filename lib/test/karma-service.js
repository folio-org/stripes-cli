...
module.exports = class KarmaService {
  ...
  generateKarmaConfig(webpackConfig, karmaOptions) {
    ...
    // set webpack's watch/cache features through karma config.
    // these features are unnecessary in the CI envirtonment, so they are turned
    // off my default. They can be enabled via command-line options.
    // these options, if undefined, are set to true.
    const watch = karmaOptions?.watch ? undefined : false;
    const cache = karmaOptions?.cache ? undefined : false;

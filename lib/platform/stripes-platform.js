...
const { cliResolve, cliAliases } = require('../webpack-common');
...
module.exports = class StripesPlatform {
  ...
  applyVirtualAppPlatform(moduleName) {
    ...
    const aliases = this.aliasService.getValidatedConfigAliases();
  ...
  getWebpackOverrides(isLocalCoreAvailable) {
    ...
    overrides.push(cliResolve(isLocalCoreAvailable, stripesCoreAlias));
    overrides.push(cliAliases(this.aliases));
    return overrides;
  }

...
module.exports = class DevelopmentEnvironment {
  ...
  selectNewModules(selectedModules) {
    ...
    this.platformDirs = this.validModules.filter(mod => mods.platforms.includes(mod));
  }
    ...
    return kopy({
      actions() {
        return [{
          ...
          files: '**',
          cwd: templateDir,
          data: cliConfigDefaults
        }];
      }
    }).run({ outDir: this.projectDir });
  }
  ...
  installDependencies() {
    ...
    if (this.isYarnWorkspace) {
      ...
      installs = packageManager.install(this.projectDir);
      ...
      this.getModulePaths()
        ...
        .forEach((moduleDir) => {
          installs = installs.then(() => packageManager.install(moduleDir));
        });
    } else {
      // For a platform without workspace, we need to install each cloned module separately
      this.getModulePaths().forEach((moduleDir) => {
        installs = installs ? installs.then(() => packageManager.install(moduleDir)) : packageManager.install(moduleDir);
      });
    }
    return installs;
  }

  ...
  _initializePlatform() {
    return new Promise((resolve) => {
      if (!this.platformDirs.length) {
        ...
        const template = path.join(__dirname, '..', '..', 'resources', 'platform');
        const target = path.join(this.projectDir, 'dev-platform');
        copy(template, target).then(() => {
          const packageJson = require(path.join(target, 'package.json')); // eslint-disable-line
          packageJson.dependencies = this.validModules.reduce((obj, mod) => {
            obj[toFolioName(mod)] = 'latest';
            return obj;
          }, {});

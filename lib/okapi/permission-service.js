...
module.exports = class PermissionService {
  ...
  async assignAllTenantPermissionsToUser(tenant, username) {
    const moduleService = new ModuleService(this.okapi);

    const userPermissions = await this.listPermissionsForUser(username);
    const tenantModuleIds = await moduleService.listModulesForTenant(tenant);
    const modulePermissions = await moduleService.listModulePermissions(tenantModuleIds, false);

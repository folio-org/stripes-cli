...
export default class ModuleService {
  ...
  listModulePermissions(moduleDescriptorIds, expand = false) {
    return this.viewModuleDescriptors(moduleDescriptorIds)
      .then(moduleDescriptors => _.flatMap(moduleDescriptors, descriptor => descriptor.permissionSets || []))
      .then(permissions => {
        const allPermissionNames = permissions.map(perm => perm.permissionName);
        const subPermissionNames = _.flatMap(permissions, perm => perm.subPermissions || []);
